...
def upload_file_to_google_drive(service, filename, file_bytes, mime_type="application/pdf"):
    ...
    This is generally more reliable for smaller files like payslips.
    Raises an exception on failure for the caller to handle.
    """
    try:
        file_metadata = {"name": filename, "parents": [GOOGLE_DRIVE_FOLDER_ID]}
        # MediaIoBaseUpload without 'resumable=True' lets the client library choose the best
        # upload method, which is typically a single, non-resumable request for small files.
    ...
    with tab_drive:
        if st.session_state.user_prefs.get("enable_drive_upload", True):
            ...
            if st.button("⬆️ Upload Selected to Google Drive", key="upload_selected_button", disabled=not selected_for_upload):
                ...
                if service:
                    # --- NEW: Robust Upload Logic with Retries ---
                    MAX_RETRIES = 3
                    RETRY_DELAY_SECONDS = 2
                    total_to_upload = len(selected_for_upload)
                    
                    st.info(f"Starting upload of {total_to_upload} files to Google Drive...")
                    upload_progress_bar = st.progress(0, text="Initializing upload...")
                    ...
                    for idx, item in enumerate(selected_for_upload):
                        ...
                        progress_fraction = idx / total_to_upload
                        base_progress_text = f"File {idx + 1}/{total_to_upload}: {filename}"
                        upload_progress_bar.progress(progress_fraction, text=f"{base_progress_text} - Queued")

                        if key in st.session_state.uploaded_file_keys_log:
                            item['upload_status_detail'] = "Skipped (Already Logged)"
                            st.toast(f"⏭️ Skipping {filename} (already uploaded).")
                            continue
                        ...
                        for attempt in range(1, MAX_RETRIES + 1):
                            try:
                                attempt_text = f" - Attempt {attempt}/{MAX_RETRIES}"
                                upload_progress_bar.progress(progress_fraction, text=base_progress_text + attempt_text)
                                st.toast(f"Uploading {filename} (Attempt {attempt})...", icon="🚀")
                                ...
                                st.toast(f"✅ Success: {filename}", icon="✅")
                                uploaded_successfully = True
                                break  # Exit retry loop

                            except Exception as e:
                                last_error = str(e)
                                st.warning(f"Attempt {attempt} failed for {filename}: {last_error}")
                                if attempt < MAX_RETRIES:
                                    retry_text = f" - Retrying in {RETRY_DELAY_SECONDS}s..."
                                    upload_progress_bar.progress(progress_fraction, text=base_progress_text + retry_text)
                                    time.sleep(RETRY_DELAY_SECONDS)
                        
                        if not uploaded_successfully:
                            st.error(f"❌ Failed to upload {filename} after {MAX_RETRIES} attempts. Skipping.")
                            item['upload_status_detail'] = f"Failed: {last_error}"

                    upload_progress_bar.progress(1.0, text=f"Upload process finished for {total_to_upload} files.")
                    time.sleep(2)
                    upload_progress_bar.empty()
                    st.success(f"Google Drive upload process complete. {total_to_upload} files were processed.")
                    # --- End of Robust Upload Logic ---
