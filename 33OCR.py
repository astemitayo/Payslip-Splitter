...
def extract_text_from_page(page):
    ...
    if not text or len(text.strip()) < 20:  # weak extraction → OCR fallback
...
def extract_ippis_and_period(text):
    ...
    # Period detection (common formats)
...
st.title("📄 Payslip Processor (Hybrid + Retry Upload)")
    ...
    if not uploaded_pdf or not sa_file or not folder_id:
        ...
        with st.spinner("Processing... Please wait."):
            ...
            for file_path in processed_files:
                ok = safe_upload_to_drive(service, folder_id, file_path, progress_bar)
                uploaded_count += 1
                progress_bar.progress(int((uploaded_count / total) * 100))
                time.sleep(0.5)

            st.success(f"🎉 Completed! Uploaded {uploaded_count} files to Drive.")
